...
- `pdf_combine/marking.py` - stamps merged output with a private `/PDFMergeNMark` Info-dictionary key plus a `pdfmerge <version>` `/Producer`, and detects it again (`is_our_output`). This exists because a merged PDF is otherwise indistinguishable from a source PDF, so a second `--here` run in the same folder would swallow the first run's output. **Only folder scans consult it**: `find_pdfs` filters, everything named explicitly does not. `is_our_output` swallows read errors and answers False, so an unreadable file stays in the list and fails loudly at merge time instead of vanishing from the scan.
- `pdf_combine/updates.py` - the "newer release available" notice. Called from `cli()`'s `finally`, so it runs after the work and cannot delay or fail a merge. Every path is silent on error, results are cached for a day under `%LOCALAPPDATA%\pdfmerge`, it only prints to a terminal, and it never self-upgrades (Windows locks the running `pdfmerge.exe`). `check_for_update` takes injectable `now`/`fetch`/`cache_file` so tests never touch the network. Needs the repo to be **public** - the releases API 404s otherwise, which the code treats as "no update".
